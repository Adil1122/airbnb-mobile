...
class _EditServiceScreenState extends State<EditServiceScreen> {
  ...
  final List<String> _categories = [
    'Photography', 'Personal Chef', 'Fitness', 'Music Lessons',
    'Home Cleaning', 'Pet Care', 'Tutoring', 'Massage',
  ];
  ...
  void initState() {
    ...
    _locationCtrl = TextEditingController(text: s.subtitle);
    ...
    _selectedDuration = _durations.contains(s.duration) ? s.duration : _durations[1];
    _maxAdults = s.guests > 0 ? s.guests : 4;
    _maxChildren = 0;
    _maxInfants = 0;
    _currentImageUrl = s.imageUrl;
    _availableFrom = s.availableFrom ?? DateTime.now();
    _availableTo = s.availableTo ?? DateTime.now().add(const Duration(days: 90));
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final updated = await _service.updateService(widget.service.id, {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Edit Service', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
        actions: [
          if (_hasChanges)
            Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildSection('Basic Info', [
              ...
              _buildTextField('Title', _titleCtrl, hint: 'e.g. Professional photography session', maxLength: 60),
              _buildTextField('Location', _locationCtrl, hint: 'e.g. Lahore, Pakistan'),
            ]),
            const Divider(height: 1),
            _buildSection('About', [
              _buildTextField('Description', _descriptionCtrl,
                  hint: 'Describe what guests will get from this service...', maxLines: 5, maxLength: 500),
              _buildDropdownField('Duration / Pricing unit', _selectedDuration, _durations,
                  (v) { setState(() { _selectedDuration = v!; _hasChanges = true; }); }),
            ]),
            const Divider(height: 1),
            _buildSection('Capacity', [
